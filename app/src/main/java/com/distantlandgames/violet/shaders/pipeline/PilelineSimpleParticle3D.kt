...
class PipelineSimpleParticle3D : PipelineBase() {
    ...
    private var alphaChannel = AttributePackage()

    override fun initialize(context: Context) {
        // 1: Get shader source
        var vertexShaderSource = AssetHelper.readTextFileFromRawResource(context, R.raw.vertex_shader_simple)
        var fragmentShaderSource = AssetHelper.readTextFileFromRawResource(context, R.raw.fragment_shader_simple_particle)
        ...
        alphaChannel = getUniformHandlesFromNames(arrayOf("u_Fade"))
    }
    ...
    fun setAlpha(alpha: Float) {
        setData(alphaChannel.data[0].attributeHandle, alpha)
    }
}